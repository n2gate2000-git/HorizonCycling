...
## 1.5 ２つの動作モード（アーケードモードとシミュレーションモードの違い）

本システムには、ユーザーの目的や好みに応じて選べる2つの異なる操作マッピングモードが搭載されています。それぞれの役割と設計思想の比較は以下の通りです。

### 動作モード比較一覧

| 比較項目 | アーケードモード (Arcade Mode) | シミュレーションモード (Simulation Mode) [推奨] |
| :--- | :--- | :--- |
| **設計思想** | **「ペダルを踏んだ分だけ、スーパーカーをぶっ飛ばす爽快感」** | **「ゲーム内の車を、まるで自分の自転車のように走らせる一体感」** |
| **制御ロジック** | **パワー比例直結制御（W ➔ アクセル開度）**<br/>ユーザーのペダリングパワーを基準FTP値で割り、アクセル開度（0.0〜1.0）へダイレクトに反映します。 | **自転車物理モデル ＆ 車速PID追従制御**<br/>パワーと道路勾配から「本来自転車で出るべき目標速度」を計算し、車速がその速度に一致するようアクセルを自動微調整します。 |
| **最高速度の限界** | **制限なし**（車の限界性能まで）<br/>200Wで漕ぎ続けるだけで、車速は時速200km以上まで簡単に到達します。 | **自転車の最高速度に調律**（実走感重視）<br/>どんなスーパーカーに乗っていても、平地で200Wなら時速30km/h程度、全力スプリントでも時速60km/h程度に制限されます。 |
| **坂道での挙動** | **ユーザーの踏力に完全依存**（要自力パワーアップ）<br/>坂道に入って車速が落ちてもアクセルは自動で増えないため、ユーザーが自力で強く踏み込まないと失速・停止します。 | **PIDがアクセルを自動アシスト**（自動スロットル）<br/>上り坂に入って車速が落ちると、PID制御が検知して自動的に車のアクセルを100%まで踏み込んで車速を維持しようとします。 |
| **スマート機能** | なし（シンプルな直結制御） | **仮想ギアダウン**（低車速時のスマートローラー自動負荷軽減）、**下り坂自動滑走**（ペダルを止めても重力で自動加速＆自動操舵アシスト維持） |
| **最適な用途** | 気軽にレースゲームを楽しみたいとき、ペダルをコントローラーの代わりに使って爽快にドライブしたいとき | バーチャルサイクリング（Zwift等の代わり）として、現実のロードバイクで実走しているようなリアルなトレーニングを行いたいとき |

---

### 各モードの詳細解説

#### 1. アーケードモード（Arcade Mode）
ユーザーの瞬時ペダリングパワー（W）をそのまま車のアクセル（Throttle: 0.0〜1.0）へリニアにマッピングします。
*   **マッピング式**: 
    $$\text{Throttle} = \text{Clamp}\left(\frac{\text{Power}}{\text{FTP}},\, 0.0,\, 1.0\right)$$
*   **特徴**:
    *   FTP（Functional Threshold Power: 基準パワー、初期設定200W）を基準とし、例えば200Wを出して漕げば車のアクセルは 100%（全開）になります。
    *   現実の空気抵抗や重量などの物理法則を一切無視し、ペダルの踏み込み量がそのままゲーム内のスロットルペダルと直結するため、ゲーム本来のスピード感とレースでの勝利を両立できます。
*   **欠点**: 
    *   坂道等で車が失速した際、自動でアクセルが追加されないため、低速での発進や坂道でペダルを一定の強さで漕ぎ続けるのが難しくなります。（※アクセルのマッピングが `HAxis1` [ハーフ軸] から `Axis1` [ノーマル軸] に修正されたため、中間出力のコントロール性は大幅に改善されていますが、坂道での失速悪循環はモード自体の仕様となります）。

#### 2. シミュレーションモード（Simulation Mode）[推奨]
本システムの真骨頂となるリアル志向のバーチャルサイクリングモードです。どんなスペックの車（たとえ時速300km出るハイパーカー）に乗っていても、車は完全に「自転車本来の物理的な性能」に調律されます。
*   **特徴**:
    *   **高精度な自転車物理モデル**: ライダーとロードバイクの総重量（75kg）、タイヤ転がり抵抗係数（0.004）、空気密度、前面投影面積と風抵抗（CdA 0.32相当）の運動方程式を、毎フレームニュートン・ラフソン法で解いて「本来自転車であれば出るはずの目標速度（Target Speed）」をリアルタイムに計算します。
    *   **PID制御による追従**: 計算された目標速度（例: 平地150Wなら約 28 km/h）に、実際のゲーム内の車速が完全に重なるよう、PIDコントローラーがゲーム側のアクセルをミリ秒単位でミリタリーレベルの精度でコントロールします。
    *   **実走感を高めるスマート機能**:
        *   **下り坂重力自動滑走（オートパイロット）**: 下り坂で足を止めても、重力による加速が自動シミュレートされ、車の自動操舵アシストを維持したままスルスルと自動滑走します。
        *   **仮想ギアダウン（スマートローギア化）**: 非力な車が激坂で悲鳴を上げて減速した際、目標速度との乖離を検知してスマートローラーの負荷を自動で軽く（ギアを落としたような状態に）し、クランクを回しやすくします。
...
#### ⑤ 停車中・極低速時のペダル負荷解放（漕ぎ出しアシスト）とデッドロック防止設計
激坂の途中で車が停車または極低速（`Speed <= 3.0 km/h`）に陥った際、最初からスマートローラーに重い坂の負荷（例：斜度10%など）がかかっていると、ペダルが重すぎて漕ぎ出すこと（発進）ができなくなります。この発進不能を防ぐため、**「ゲーム内の車速が 3.0 km/h 以下のときは、スマートローラーに送信する斜度を強制的に `0.0%` にし、負荷を完全に解放する」**という漕ぎ出しアシスト機能を備えています。

この低速アシストによる「偽の 0% 斜度」をそのまま自転車の物理エンジン（目標速度の計算）にまで入力してしまうと、物理モデルは「平地を走っている」と錯覚して目標速度を 30km/h などに跳ね上げてしまいます。その結果、実際の低速（3.0km/h以下）との莫大な速度差により「仮想ギアダウン」が限界まで誤爆してペダルが永久に FREE のまま、実車は激坂で時速 3km/h から一生加速できなくなるという**デッドロック現象**が発生していました。

これを防止するため、本システムは**「目標速度を計算する自転車の物理エンジンには、低速補正（0%化）が入っていない本来の難易度適用後勾配（`difficultyGrade`）をインプットし、スマートローラーへの送信回路の直前でのみ極低速時の 0% 補正（`correctedGrade`）を適用する」**という、役割を正しく分離した疎結合設計を採用しています。これにより、停車中の漕ぎ出しの軽さを100%維持しつつ、物理エンジンは坂道相応の正しい低速目標を計算でき、デッドロックを起こさずに激坂を登坂・走破することが可能になりました。

#### ⑥ スマートローラーへ送信する物理設定パラメータ（一括シミュレーション設定）
本システムでは、スマートローラーに対して単に「斜度のみ（OpCode 0x03）」を送る簡易的な制御ではなく、Zwift等の本格的なバーチャルサイクリングアプリと同様に、**「自転車の設定値（空気抵抗係数 Cw、転がり抵抗係数 Crr）と斜度を一括送信する（OpCode 0x11 - Set Indoor Bike Simulation Parameters）」**という高度なFTMS制御プロトコルを採用しています。

これにより、スマートローラー内部のマイコンが送信された物理特性を忠実にシミュレートし、極めて実走に近い正確なペダル負荷（路面の重さ）を再現することができます。

スマートローラーへ送信されるデータ構造（FTMS OpCode 0x11）は以下の通りです：

| 送信データ項目 | 設定値（定数） | BLE送信時の値（スケーリング） | 役割・物理的意味 |
| :--- | :--- | :--- | :--- |
| **OpCode** | `0x11` | `0x11` (1 byte) | シミュレーションパラメータ一括設定コマンド |
| **Wind Speed**（風速） | `0.0 m/s` | `0` (2 bytes, SINT16) | 無風状態として固定（単位: 0.001 m/s） |
| **Grade**（送信斜度） | 可変（%） | `Grade * 100` (2 bytes, SINT16) | 補正後のスマートローラー制御斜度（単位: 0.01%） |
| **Crr**（転がり抵抗） | `0.004` (ロード用タイヤ) | `40` (1 byte, UINT8) | タイヤと道路の転がり抵抗係数（単位: 0.0001、`0.004 * 10000 = 40`） |
| **Cw**（空気抵抗係数） | `0.51 kg/m` | `51` (1 byte, UINT8) | サイクリストの前面投影面積を考慮した風抵抗係数（単位: 0.01 kg/m、`0.51 * 100 = 51`） |
