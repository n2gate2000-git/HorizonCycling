...
* **基準FTP（Functional Threshold Power）の設定および次回自動保持**
  ...
  設定されたFTP値は `config.json` に自動保存され、次回起動時も自動的に読み込まれてアーケードモードのパワーマッピング（0〜100%）の基準値として使用されます。

### v0.4
* **ペダルパワー運動制御ブレーキ機能 (3ゾーンペダルコントロール) の実装**
...
* **モード・負荷難易度・基準FTPの次回自動引き継ぎ**
  初回セットアップで設定した「基準FTP（Functional Threshold Power）」や起動時に選択した「動作モード（Simulation / Arcade）」、「負荷難易度（Difficulty）」が `config.json` に自動保存され、次回起動時のデフォルト値として読み込まれます。
...
* **x360ce (Xbox 360 Controller Emulator)**: vJoy入力をXbox 360コントローラー入力に変換するエミュレーター。
  ...
  * ~~x360ce 公式ダウンロードサイト~~ Glitchtest51氏による[x360ceフォーク版](https://github.com/Glitchtest51/x360ce/releases/tag/v4.17.15.1) からダウンロードします。公式サイトのモジュールは最新のVisualC++の再頒布パッケージに対応していません。
...
1. Glitchtest51氏による[x360ceフォーク版](https://github.com/Glitchtest51/x360ce/releases/tag/v4.17.15.1) の `x360ce.exe` をダウンロードし、任意のフォルダに保存して起動します。
...
5. x360ceのコントローラーマッピング画面にて：
   * **「Right Trigger (RT)」** (アクセル) に vJoy の `Axis X` (または `Axis 1`) を割り当てます。
   * **「Left Trigger (LT)」** (ブレーキ) に vJoy の `Axis Y` (または `Axis 2`) を割り当てます。
6. 設定後、ペダルを回す（または `Space` / `B` キーを押す）と、x360ceの「RT」および「LT」インジケーターが 0%〜100% で反応することを確認します。
...
| **ドライビングライン** | `オフ` | コース上に最適なドライビングラインを表示 |
...
* **特におすすめの車種**:
  * **P50 (1962 Peel P50)**: 超軽量・極低馬力のマイクロカー。自転車の速度域やマイルドな加速フィールに最も相性が良く、安定した素晴らしい連動体験が得られます。本アプリのアーケードモードで他のP50と競いあうには、少しのカスタムアップグレードを施してください。相手はエンジン、こちらは人力なので、少し性能を良くしたほうが良いです（脚力に自信がある方は不要かもしれませんが）。アップグレードショップでカスタマイズします。おすすめのカスタマイズは下記になります。
    * エンジン - 吸気系 - 吸気系（スポーツ仕様）
    * 駆動系 - トランスミッション - トランスミッション（レース仕様）
...
6. 起動すると、コマンドプロンプトのようなコンソール画面（黒い画面）が開き、初期設定プロンプトが表示されます。
   * **[初回起動時のデバイスセットアップ (CLI)]**
     ...
     * 続いて、**基準FTP（Functional Threshold Power: W）の入力プロンプト**が表示されます。ご自身の基準FTPを入力すると、`config.json` に保存され、アーケードモード使用時のパワーマッピング（0〜100%）に利用されます。（※未入力・Enterのみでデフォルトの200Wが設定されます）。
     * 選択・入力した設定は保存され、次回以降は自動的に読み込まれます。（※設定を変更したい場合は、コマンドライン引数 `--setup-sensors` を付けて起動し直してください）。
